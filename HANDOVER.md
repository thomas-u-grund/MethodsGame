...
- **Narrated trailers.** 14 narrator clips (LibriVox **David Clarke**, exag 0.5 / cfg 0.5 — `vo-narr-opening-*.mp3`, `vo-narr-act2-*.mp3`, the two title cards read "Act One. The Question." / "Act Two. Apparently, we need data."). Panels take a `voice:` key; both players (boot trailer and `CODEBOOK_PLAY_INTERLUDE`) play it and **auto-advance when the clip ends**, while click/Space still skip (and stop that panel's narration). Clips are in the act1/act2 preload lists.
